...
import com.kmarinos.businessemaildemo.core.providers.PartsTextProvider;
import com.kmarinos.businessemaildemo.core.exceptions.NoDeferredTextProviderException;
import com.kmarinos.businessemaildemo.core.exceptions.TextProviderDoesNotImplementMethodException;
...
public abstract class ContextualPartsTextProvider<R, T extends MailContext> extends PartsTextProvider<T> implements ConditionalContextualTextProvider<R> {
    ...
    protected <X> X defer() {
        ...
        var otherTextProvider = EmailTextBuilder.getStandByTextProviders().get().getNext(caller.getClassName()).orElseThrow();

        return runNextAvailableHomonymousMethod(caller.getClassName(),caller.getMethodName(), otherTextProvider, new Object[]{},
                previousTextProvider -> EmailTextBuilder.getStandByTextProviders().get().getNext(previousTextProvider.getClass().getName())
